...
namespace Csla8ModelTemplates.Dal.SqlServer
{
    ...
    public static class SqlServerSeeder
    {
        ...
        {
            await context.Database.ExecuteSqlRawAsync("DBCC CHECKIDENT ('Teams', RESEED, 0);");
            await context.Database.ExecuteSqlRawAsync("DBCC CHECKIDENT ('Players', RESEED, 0);");
            await context.Database.ExecuteSqlRawAsync("DBCC CHECKIDENT ('Groups', RESEED, 0);");
            await context.Database.ExecuteSqlRawAsync("DBCC CHECKIDENT ('Persons', RESEED, 0);");
            await context.Database.ExecuteSqlRawAsync("DBCC CHECKIDENT ('Folders', RESEED, 0);");
        }
